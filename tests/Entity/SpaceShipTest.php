...
class SpaceShipTest extends \PHPUnit\Framework\TestCase
{
  ...
  {
    $module = new Module(14);
    $spaceShip = new SpaceShip([$module]);
    $this->assertEquals($spaceShip->requiredFuelToLaunch(), 2);
  }

  public function testFuelNeedIs654IfModuleMassIs1969()
  {
    $module = new Module(1969);
    $spaceShip = new SpaceShip([$module]);
    $this->assertEquals($spaceShip->requiredFuelToLaunch(), 966);
  }

  public function testFuelNeedIs33583IfModuleMassIs100756()
  {
    $module = new Module(100756);
    $spaceShip = new SpaceShip([$module]);
    $this->assertEquals($spaceShip->requiredFuelToLaunch(), 50346);
  }

  public function testFuelNeededInPuzzleTest(){
    $modules = [];
    foreach (self::PUZZLE_INPUT as $mass){
      $modules[] = new Module($mass);
    }
    $spaceShip = new SpaceShip($modules);
    $this->assertEquals($spaceShip->requiredFuelToLaunch(), 4900568);
  }
}